...
int main() {
    ...
    std::ofstream iter("tp1-exo2-iterations.txt");
    ...
    if (carre_0025) { 
        ...
        double h=0.025;
        Mesh2D Omega;
        Read(Omega, "maillages/carre-0025.mesh");
        ...
        // Etude de tp1-exo2-convergence du GC
        int maxIter = 250;
        int k_stop = maxIter;
        ...
        for (int k=1; k<maxIter; k++) {     
            ...
            auto err = uh - ucg;
            Conv[k] = ((K(err)|err) + (M(err)|err))/((K(u)|u) + (M(u)|u));
            if (Conv[k] < 1e-6) {
        ...
        // Sauvegarde
        std::string filename = "tp1-exo2-convergence_0025.txt";
        std::ofstream myfile;
    ...
    if (carre_001) { 
        ...
        // Sauvegarde
        std::string filename = "tp1-exo2-convergence_001.txt";
        std::ofstream myfile;
        myfile.open(filename);
        for (int i=1; i<maxIter; i++){
            myfile << Conv[i] << "\n";
    ...
    if (carre_0005) {
        ...
        double h = 0.005;
        Mesh2D Omega;
        Read(Omega, "maillages/carre-0005.mesh");
        ...
        // Sauvegarde
        std::string filename = "tp1-exo2-convergence_0005.txt";
        std::ofstream myfile;
        myfile.open(filename);
        for (int i=1; i<maxIter; i++){
            myfile << Conv[i] << "\n";
    ...
    if (carre_00025) { 
        ...
        double h = 0.0025;
        Mesh2D Omega;
        Read(Omega, "maillages/carre-00025.mesh");
        ...
        // Sauvegarde
        std::string filename = "tp1-exo2-convergence_00025.txt";
        std::ofstream myfile;
        myfile.open(filename);
        for (int i=1; i<maxIter; i++){
            myfile << Conv[i] << "\n";
