...
int main(int argc, char* argv[])
{
    ...
    // indice globaux, les nodes vont etre update entre [|i_start, i_end|]
    // le process possede les nodes [|i_start-1, i_end+1|]
    int i_start = 1+ decalage;    int i_end = i_start + Nx_local -1;

    int size_flank = ((Ny+2)/2)+1;  // taille des vecteurs de com
    ...
    for(int i=0; i<Nx_local+2;i++)
    {
        ...
        x = (i_global)*dx;
        U_loc[i*(Ny+2)]=        U0;         // bas
        U_loc[i*(Ny+2)+(Ny+1)]= U0;         // haut
    }

    if(myRank==0){          for(int j=0; j<Ny+2;j++){y = j*dy;  U_loc[j] =                    V(y);}}  // gauche 
    if(myRank==(nbTask-1)){ for(int j=0; j<Ny+2;j++){y = j*dy;  U_loc[(Nx_local+1)*(Ny+2) +j] = U0;}}  // droite
    }
    // =============================================================================
    
    vector<double> U_loc_Next =U_loc;       // vecteur de la solution du probleme au temps t+
    vector<double> U_left_R(size_flank), U_right_R(size_flank), U_left_N(size_flank), U_right_N(size_flank); // vecteurs de com 
    ...
    {   
        ...
        if(myRank ==0){
            if(progress * Time/precision<t){cout<<"progress : "<<(t*100)<<"% \n"; progress +=1;}
        }
        // =============================================================================
        // =============================================================================
        ...
        if(myRank>0){
            ...
            // charge les vecteurs de com
            for(int i=0;     i<Ny+2;     i++){
                if(is_red(i_start,i))
                { 
                    if(i%2==0){ U_left_R[i/2]    = U_loc[Ny+2+i]; }
                    if(i%2==1){ U_left_R[(i+1)/2]= U_loc[Ny+2+i]; }
                } 
            }

            // envoie et recoie les vecteurs de com
            ...
            // decharge les vecteurs de com
            for(int i=0;     i<Ny+2;     i++){
                if(is_red(i_start-1,i))
                { 
                    if(i%2==0) U_loc[i]= U_left_R[i/2];       
                    if(i%2==1) U_loc[i]= U_left_R[(i+1)/2];   
                }
            }
        ...
        if(myRank<nbTask-1){
            ...
            for(int i=0;     i<Ny+2;     i++){
                if(is_red(i_end,i))
                {
                    if(i%2==0){ U_right_R[i/2]    =U_loc[(Nx_local)*(Ny+2)+i];  }
                    if(i%2==1){ U_right_R[(i+1)/2]=U_loc[(Nx_local)*(Ny+2)+i];  }
                }
            }
            ...
            for(int i=0;     i<Ny+2;     i++){
                if(is_red(i_end+1,i))
                {
                    if(i%2==0) U_loc[(Nx_local+1)*(Ny+2)+i]= U_right_R[i/2];    
                    if(i%2==1) U_loc[(Nx_local+1)*(Ny+2)+i]= U_right_R[(i+1)/2]; 
                } 
            }
        ...
        for(int i=1; i<Nx_local+1;i++)
        {
            ...
            for(int j=1;j<Ny+1;j++)
            {
                ...
                {
                    y = j*dy;                
                    U_loc_Next[i*(Ny+2)+j] = cste*(U_loc[(i+1)*(Ny+2)+j]*dx_2 + U_loc[i*(Ny+2)+(j+1)]*dy_2);  // termes non update de GS
                    U_loc_Next[i*(Ny+2)+j] += cste*(U_loc[(i-1)*(Ny+2)+j]*dx_2+ U_loc[i*(Ny+2)+(j-1)]*dy_2);  // termes update de GS
                    U_loc_Next[i*(Ny+2)+j] += -cste* f(x,y);                                                  // terme de bord
        ...
        if(myRank>0){
            ...
            for(int i=0;     i<Ny+2;     i++)
            {
                ...
                {
                    if(i%2==0){ U_left_N[i/2]    =U_loc[Ny+2+i];}
                    if(i%2==1){ U_left_N[(i+1)/2]=U_loc[Ny+2+i];}
                }
            }
            ...
            for(int i=0;     i<Ny+2;     i++)
            {
                ...
                {
                    if(i%2==0) U_loc[i]= U_left_N[i/2];       
                    if(i%2==1) U_loc[i]= U_left_N[(i+1)/2];   
                }
            }
        } 
        ...
        if(myRank<nbTask-1){
            ...
            for(int i=0;     i<Ny+2;     i++)
            {
                ...
                {
                    if(i%2==0){ U_right_N[i/2]    =U_loc[(Nx_local)*(Ny+2)+i];}
                    if(i%2==1){ U_right_N[(i+1)/2]=U_loc[(Nx_local)*(Ny+2)+i];}
                }
            }
            ...
            for(int i=0;     i<Ny+2;     i++)
            {
                ...
                { 
                    if(i%2==0) U_loc[(Nx_local+1)*(Ny+2)+i]= U_right_N[i/2];     
                    if(i%2==1) U_loc[(Nx_local+1)*(Ny+2)+i]= U_right_N[(i+1)/2]; 
        ...
        for(int i=1; i<Nx_local+1;i++)
        {
            ...
            for(int j=1;j<Ny+1;j++)
            {
                ...
                U_loc_Next[i*(Ny+2)+j] = cste*(U_loc_Next[(i+1)*(Ny+2)+j]*dx_2 + U_loc_Next[i*(Ny+2)+(j+1)]*dy_2);  // termes non update de GS
                U_loc_Next[i*(Ny+2)+j] += cste*(U_loc_Next[(i-1)*(Ny+2)+j]*dx_2+ U_loc_Next[i*(Ny+2)+(j-1)]*dy_2);  // termes update de GS
                U_loc_Next[i*(Ny+2)+j] += -cste* f(x,y);                                                            // terme de bord
...
// ===========================================================================================
    ...
    if (myRank == 0) 
    {
        ...
        for (int i=0; i<Nx+2; i++) 
        {   for (int j=0; j<Ny+2; j++){myfile << U_global[i*(Ny+2) + j] << "\n";}
        }
        myfile.close();
        cout << "Sauvegarde dans u_sol.txt effectuée." << endl;

        if(EtudeErr)    // send the infinity norm of the error
        {
    ...
    {
        // Send its part of the solution to the "center" node
