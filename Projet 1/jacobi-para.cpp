...
// 
    ...
    if (myRank == 0) {
        cout << "===============================" << endl;
        cout << "Paramètres du problème:" << endl;
        cout << "Domaine: (0," << a << ") x (0," << b << ")" << endl;
        cout << "Maillage: Nx = " << Nx << ", Ny = " << Ny << endl;
        cout << "===============================" << endl;
    };
    ...
    while (iteration < Nmax && maxResidu > tol) {
        maxResiduLoc = 0.0;
        if (myRank > 0) {
            MPI_Sendrecv(&u[1*(Ny+2)], Ny+2, MPI_DOUBLE, myRank-1, 0,
                         &u[0*(Ny+2)], Ny+2, MPI_DOUBLE, myRank-1, 0,
                         MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        if (myRank < nbTask-1) {
            MPI_Sendrecv(&u[Nx_local*(Ny+2)], Ny+2, MPI_DOUBLE, myRank+1, 0,
                         &u[(Nx_local+1)*(Ny+2)], Ny+2, MPI_DOUBLE, myRank+1, 0,
                          MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
